...
//            
 ...
 // 向文档中添加 一个图片，  
    ...
    public void insetImage(String bookMarkKey,String ImagePath){
    	  Dispatch activeDocument = MsWordApp.getProperty("ActiveDocument").toDispatch();   
          Dispatch bookMarks = MsWordApp.call(activeDocument, "Bookmarks")  
                  .toDispatch();  
          boolean bookMarkExist1 = Dispatch.call(bookMarks, "Exists",  
          ...
          if (bookMarkExist1 == true) {  
              System.out.println("已找到书签 " + bookMarkKey);  
              Dispatch image = Dispatch.get(selection,"InLineShapes").toDispatch(); 
              Dispatch.call(image,"AddPicture",ImagePath); 
...
//    public void insertImage1(String jpegFilePath){
//		  Dispatch selection = Dispatch.get(MsWordApp, "Selection").toDispatch();  
//	      Dispatch inLineShapes = Dispatch.get(selection, "InLineShapes").toDispatch();   
//	      Dispatch picture = Dispatch.call(inLineShapes, "AddPicture", jpegFilePath).toDispatch();   
//	      //选中图片  
